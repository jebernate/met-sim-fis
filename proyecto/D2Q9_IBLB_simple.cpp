...
void Engine::GetAllForces(double gx, double gy, Body *Bodies, LatticeBoltzmann &Lattice)
{
    ...
    for (i = 0; i < N; i++)
        GetLatticeForces(0.0, 0.0, Bodies[i], Bodies, Lattice);
...
int main(void)
{
    ...
    double rhom0 = 2.0, R0 = 15, rho0 = 1.0;
    double gx = 0.0, gy = -0.1 * (100.0 * nu / (2 * M_PI * R0)); //(10*nu/(4*R0))
    int i, t, tmax = 800;
    double dt = 1.0;

    char Filename[] = "data_fluid_simple.dat";
    char FilenameBody[] = "data_simple.dat";
    std::ofstream MyFile(Filename);
    MyFile.close();
    std::ofstream MyFileB(FilenameBody);
    MyFileB.close();
    double Vy0 = -1e-3 * (100.0 * nu / (2 * M_PI * R0));

    // Start

    for (i = 0; i < N; i++)
        Bodies[i].Start(Lx / 2, Ly / 2, 0.0, Vy0, rhom0, R0);
    ...
    for (t = 1; t <= tmax; t++)
    {
        ...
        // Print
        Bodies[0].Print(FilenameBody, t);
        // if(t%10==0) std::cout << "t = " << t << std::endl;
    }
    Fluid.Print(Filename, 0, 0, 1.0, Bodies);
